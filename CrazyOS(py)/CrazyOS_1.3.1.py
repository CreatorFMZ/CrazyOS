...
pygame.display.set_caption("CrazyOS 1.3.1 - Python 操作系统模拟器")
...
class Window:
    ...
    def handle_keyboard_input(self, event):
        if self.app_type == "deeplost" and self.editing:
            if event.key == pygame.K_BACKSPACE:
                if self.cursor_pos_deeplost > 0:
                    self.input_text = self.input_text[:self.cursor_pos_deeplost-1] + self.input_text[self.cursor_pos_deeplost:]
                    self.cursor_pos_deeplost -= 1
            elif event.key == pygame.K_DELETE:
                if self.cursor_pos_deeplost < len(self.input_text):
                    self.input_text = self.input_text[:self.cursor_pos_deeplost] + self.input_text[self.cursor_pos_deeplost+1:]
                ...
                self.input_text = self.input_text[:self.cursor_pos_deeplost] + event.unicode + self.input_text[self.cursor_pos_deeplost:]
                self.cursor_pos_deeplost += 1
            ...
            if self.show_rename_dialog:
                if event.key == pygame.K_BACKSPACE:
                    ...
                    if self.rename_input.strip():
                        self.filename = self.rename_input + (".txt" if not self.rename_input.endswith(".txt") else "")
                        self.show_rename_dialog = False
                        self.save_document()
                elif event.unicode.isprintable():
    ...
    def rename_document(self):
        ...
        self.rename_input = self.filename.replace(".txt", "")
    ...
    def draw(self, surface):
        ...
        if self.is_maximized:
            ...
            pygame.draw.rect(surface, (240, 240, 240), (maximize_button.centerx-5, maximize_button.centery-5, 10, 10), 1)
        ...
        pygame.draw.line(surface, (240, 240, 240), (minimize_button.centerx-6, minimize_button.centery+3),
                         (minimize_button.centerx+6, minimize_button.centery+3), 2)
    ...
    def draw_explorer(self, surface):
        for item_i, item in enumerate(self.content):
            y_pos = self.y + 60 + item_i * 40
    ...
    def draw_settings(self, surface):
        ...
        if self.selected_setting == 0:
            fullscreen_rect = pygame.Rect(self.x + 50, self.y + 120, self.width - 100, 100)
            pygame.draw.rect(surface, WINDOW_TITLE_BAR, fullscreen_rect, border_radius=10)
            toggle_text = font_medium.render(f"全屏模式: {'开' if self.fullscreen_toggle else '关'}", True, TEXT_COLOR)
            surface.blit(toggle_text, (fullscreen_rect.x + 20, fullscreen_rect.y + 20))
            toggle_button = pygame.Rect(fullscreen_rect.x + 150, fullscreen_rect.y + 50, 80, 30)
            color = BUTTON_HOVER if toggle_button.collidepoint(pygame.mouse.get_pos()) else BUTTON_COLOR
            pygame.draw.rect(surface, color, toggle_button, border_radius=5)
            btn_text = font_medium.render("切换", True, TEXT_COLOR)
            surface.blit(btn_text, (toggle_button.centerx - btn_text.get_width()//2, toggle_button.centery - btn_text.get_height()//2))
        if self.show_update:
    ...
    def draw_deeplost(self, surface):
        ...
        if self.editing and self.cursor_visible:
            cursor_x = self.input_box.x + 5 + font_medium.size(self.input_text[:self.cursor_pos_deeplost])[0]
            cursor_y = self.input_box.y + 5
            pygame.draw.line(surface, TEXT_COLOR, (cursor_x, cursor_y), (cursor_x, cursor_y + 20), 2)
        ...
        wrapped_text = self.wrap_text(self.output_text, self.width - 60)
    ...
    def draw_documents(self, surface):
        ...
        for button in self.buttons:
            if button["action"] in ["save_document", "rename_document"]:
                mouse_pos = pygame.mouse.get_pos()
                ...
                btn_text = font_medium.render("保存" if button["action"] == "save_document" else "重命名", True, TEXT_COLOR)
        ...
        if self.show_rename_dialog:
            dialog_rect = pygame.Rect(self.x + 50, self.y + 150, self.width - 100, 100)
            pygame.draw.rect(surface, WINDOW_BG, dialog_rect, border_radius=10)
            pygame.draw.rect(surface, SYSTEM_BLUE, dialog_rect, 2, border_radius=10)
            prompt_text = font_medium.render("输入新文件名:", True, TEXT_COLOR)
            surface.blit(prompt_text, (dialog_rect.x + 20, dialog_rect.y + 20))
            input_rect = pygame.Rect(dialog_rect.x + 20, dialog_rect.y + 50, dialog_rect.width - 40, 30)
            pygame.draw.rect(surface, (230, 240, 250), input_rect, border_radius=5)
            input_text = font_medium.render(self.rename_input, True, TEXT_COLOR)
            surface.blit(input_text, (input_rect.x + 5, input_rect.y + 5))
            if self.cursor_visible:
                cursor_x = input_rect.x + 5 + font_medium.size(self.rename_input)[0]
                pygame.draw.line(surface, TEXT_COLOR, (cursor_x, input_rect.y + 5), (cursor_x, input_rect.y + 25), 2)
...
class Taskbar:
    def __init__(self):
        ...
        self.start_menu_items = [
            "文件管理器", "计算器", "浏览器", "DeepLost-X1", "文档", "图片", "系统设置"
        ]
    ...
    def draw(self, surface):
        ...
        if self.start_menu_open:
            menu_height = max(240, len(self.start_menu_items) * 35 + 20)
            menu_rect = pygame.Rect(10, HEIGHT - self.height - menu_height, 200, menu_height)
            pygame.draw.rect(surface, WINDOW_BG, menu_rect, border_radius=8)
            pygame.draw.rect(surface, SYSTEM_BLUE, menu_rect, 2, border_radius=8)
            for i, item in enumerate(self.start_menu_items):
                item_rect = pygame.Rect(menu_rect.x + 10, menu_rect.y + 10 + i * 35, 180, 30)
                color = BUTTON_HOVER if item_rect.collidepoint(pygame.mouse.get_pos()) else BUTTON_COLOR
                pygame.draw.rect(surface, color, item_rect, border_radius=5)
                item_text = font_medium.render(item, True, TEXT_COLOR)
                surface.blit(item_text, (item_rect.x + 10, item_rect.y + 5))
...
# 桌面图标垂直排列
icon_positions = [(100, 100 + i * 120) for i in range(7)]
desktop_icons = [
...
def update_loop():
    global running, active_window_index, active_input_window
    current_time = datetime.now()
    mouse_pos = pygame.mouse.get_pos()

    for window in windows:
        if window.app_type == "documents" or window.app_type == "deeplost":
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
                        ...
                        if event.button == 1:
                            ...
                            if current_ticks - icon.last_click < 300:
                                ...
                                window_title = {
                                    ...
                                    "documents": "文档编辑器",
                                    "settings": "系统设置",
                                    "deeplost": "DeepLost-X1",
                                    "images": "图片"
                                }.get(app_type, icon.label)
            ...
            if not icon_clicked:
                start_rect = pygame.Rect(10, HEIGHT - taskbar.height + 5, 80, 40)
                if start_rect.collidepoint(mouse_pos):
                    taskbar.start_menu_open = not taskbar.start_menu_open
                elif taskbar.start_menu_open:
                    menu_height = max(240, len(taskbar.start_menu_items) * 35 + 20)
                    menu_rect = pygame.Rect(10, HEIGHT - taskbar.height - menu_height, 200, menu_height)
                    if menu_rect.collidepoint(mouse_pos):
                        for i, item in enumerate(taskbar.start_menu_items):
                            item_rect = pygame.Rect(menu_rect.x + 10, menu_rect.y + 10 + i * 35, 180, 30)
                            if item_rect.collidepoint(mouse_pos):
                                app_type = {
                                    ...
                                    "计算器": "calculator",
                                    "浏览器": "browser",
                                    "DeepLost-X1": "deeplost",
                                    "文档": "documents",
                                    "图片": "images",
                                    "系统设置": "settings"
                                }[item]
                                new_window = Window(item, 150 + len(windows)*20, 100 + len(windows)*20, 400, 400, app_type)
                                windows.append(new_window)
                                active_window_index = len(windows) - 1
                                if item not in taskbar.app_buttons:
                                    taskbar.app_buttons.append(item)
                        ...
                        if event.button == 1:  # 左键恢复窗口
                            for j, window in enumerate(windows):
                                if window.title == btn_title:
                                    ...
                                    active_window_index = len(windows) - 1
                                    break
                        elif event.button == 3:  # 右键关闭窗口
                            for j, window in enumerate(windows):
                                if window.title == btn_title:
                                    windows.pop(j)
                                    taskbar.app_buttons.remove(btn_title)
                                    active_window_index = -1 if not windows else len(windows) - 1
                                    if active_input_window and active_input_window.title == btn_title:
                                        active_input_window = None
                                    break
                ...
                if window.is_inside(mouse_pos):
                    ...
                    for button in window.buttons:
                        if button["rect"].collidepoint(mouse_pos):
                            if button["action"] == "close":
                                ...
                                break
                            elif button["action"] == "rename_document":
                                window.rename_document()
                                break
                    ...
                    if window.is_title_bar_hit(mouse_pos) and not window.is_maximized:
                        ...
                        if window.app_type == "deeplost" and window.input_box.collidepoint(mouse_pos):
                            ...
                            window.cursor_pos_deeplost = min(len(window.input_text), 
                                max(0, int((mouse_pos[0] - window.input_box.x - 5) / font_medium.size("字")[0])))
                            ...
                            for item_i, item in enumerate(window.folder_contents[window.current_folder]):
                                ...
                                if item_rect.collidepoint(mouse_pos):
                                    ...
                                    break
                        elif window.app_type == "settings" and window.selected_setting == 0:
                            toggle_button = pygame.Rect(window.x + 200, window.y + 170, 80, 30)
                            if toggle_button.collidepoint(mouse_pos):
                                window.fullscreen_toggle = not window.fullscreen_toggle
                                pygame.display.set_mode((WIDTH, HEIGHT), pygame.FULLSCREEN if window.fullscreen_toggle else 0)
