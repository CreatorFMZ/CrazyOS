...
pygame.display.set_caption("CrazyOS 1.1 - Python 操作系统模拟器")


# 颜色定义（Windows 11风格）
BACKGROUND = (98, 142, 203)  # Windows 11 蓝色
...
class Window:
    ...
    def draw_deeplost(self, surface):
        content_rect = pygame.Rect(self.x + 20, self.y + 60, self.width - 40, self.height - 90)
        pygame.draw.rect(surface, (240, 245, 255), content_rect, border_radius=8)
        for i, line in enumerate(self.content):
            text_surface = font_medium.render(line, True, (30, 30, 60))
            surface.blit(text_surface, (content_rect.x + 20, content_rect.y + 20 + i * 30))
...
class Taskbar:
    ...
    def draw(self, surface):
        ...
        for i, btn in enumerate(self.app_buttons):
            ...
            app_text = font_medium.render(btn, True, TEXT_COLOR, 110)
...
desktop_icons = [
    ...
    Icon(460, 100, "DeepLost-X1", "deeplost"),
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
                ...
                if icon.is_hovered(mouse_pos):
                    ...
                    if event.button == 1:
                        ...
                        if current_ticks - icon.last_click < 300:
                            ...
                            # 特殊应用处理
                            if app_type == "deeplost":
                                window_title = "DeepLost-X1 V2.0"
                                new_window = Window(window_title, 150 + len(windows)*20, 100 + len(windows)*20, 600, 400, "deeplost")
                                new_window.content = [
                                    "欢迎使用 DeepLost-X1 V2.0",
                                    "这是一款由 Fanatic Star 开发的智能创作模型。",
                                    "功能：自动文本生成、诗歌创作、语言模拟",
                                    "",
                                    "提示：完整版在终端运行 deep_lost_x1.py 查看演示"
                                ]
                            ...
                            # 通用应用处理
                                ...
                                if app_type == "explorer":
                                    ...
                                    window_title = icon.label # Fallback
                            ...
                            for item_i, item in enumerate(window.content):
                                ...
                                if item_rect.collidepoint(mouse_pos):
                                    ...
                                    if item == "关于":
                                        ...
                                        window.show_about = not window.show_about # Or other action
                                    break
            ...
            if not handled and not icon_clicked:
                ...
                # 关闭设置中的关于面板
                for window in windows:
                    if window.app_type == "settings" and window.show_about and not window.is_inside(mouse_pos):
    ...
    screen.blit(date_text, (WIDTH - date_text.get_width() - 130, HEIGHT - 25))

    # 绘制鼠标指针
    pygame.draw.polygon(screen, (255, 255, 255), [mouse_pos, (mouse_pos[0]+15, mouse_pos[1]+20), (mouse_pos[0]+5, mouse_pos[1]+25), (mouse_pos[0], mouse_pos[1]+20)])
    pygame.draw.polygon(screen, (0,0,0), [mouse_pos, (mouse_pos[0]+15, mouse_pos[1]+20), (mouse_pos[0]+5, mouse_pos[1]+25), (mouse_pos[0], mouse_pos[1]+20)], 1)
